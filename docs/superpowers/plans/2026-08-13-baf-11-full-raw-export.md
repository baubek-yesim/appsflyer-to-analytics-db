...
1. Данных в `appsflyer_events_fb` нет минимум с 27.07. **Разрыв ещё восстановим**, но окно закрывается:
   наблюдаемая граница молчаливой пустоты Pull API — ~35 дней (`config.py:91`), и 27.07 упирается в
   неё примерно 31.08.
...
приходит как валидный ответ и молча недогружает окно.
...
**Q1. Подтвердить последствия, а не смысл.** Текст п.4 однозначен — сняты **обе** переменные,
