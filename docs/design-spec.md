...
| **Dual attribution: a retargeting-attributed event also appears in the UA report** (~14 twin purchases/month measured — issues #7/#46/#47) | A purchase can appear under both `attribution_type` values | **Accepted by design since 2026-07-09 (issue #47, data-analytics decision on #46, reversing #7's filter):** all UA rows load, including `Is Primary Attribution = false`; dedup follows Mark's key (`event_time, event_name, appsflyer_id, attribution_type`), under which the twin rows are two legitimate dimension entries. Cross-attribution sums count such purchases in both dimensions — matching the reference extract's semantics. Per-attribution metrics are unaffected. **As of issue #55**, the `Is Primary Attribution` flag is persisted as `is_primary_attribution` on every row, and a `<table>_deduped` view (`sql/create_view.sql`, `appsflyer-pipeline create-view`) gives a de-duplicated read on top of the still-unfiltered base table: per `(event_time, event_name, appsflyer_id)`, it keeps the `is_primary_attribution = true` row of a non_organic/retargeting pair and drops the twin, while singleton purchases (no cross-attribution twin) pass through untouched regardless of their flag. This resolves the ~1.2% cross-attribution revenue double-count (43 pairs / 3,467 rows, 575.34 of 46,909.23 measured live 2026-07-10) at query time, without reintroducing `#7`'s load-time drop of non-FB-primary singles. Base-table queries are unchanged and still double-count by design; the view is the canonical de-duplicated read. |
