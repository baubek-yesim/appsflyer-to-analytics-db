...
| **Two distinct purchases inside one second by one user** (measured live 2026-08-13: one pair, 3 vs 4 EUR, in a 242-row window) | Mark's dedup key (`event_time, event_name, appsflyer_id, attribution_type`) cannot separate them — AppsFlyer timestamps are second-granular | Both rows are kept, with a WARNING naming the first conflicting key. Until 2026-08-13 this raised `TransformError` instead, which cost the **entire** window: the July 15-26 gap fill loaded 3/4 windows and skipped `com.yesimmobile/non_organic` in full. Exact-duplicate (byte-identical) rows still collapse. |
